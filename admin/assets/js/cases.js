...
var CASES_UI = { mode: 'dashboard', caseId: null, tab: 'milestones', selectedTaskId: null, filters: { q: '', service: '', owner: '', risk: '' }, myWork: '' };
...
/* Mongo/Mongoose references may arrive as strings or populated objects. */
function _cxRefId(value) {
  if (value && typeof value === 'object') {
    return value.id != null ? value.id : (value._id != null ? value._id : '');
  }
...
function _cxLoad() {
  return Promise.all([
    DataAPI.getProjects(),
    DataAPI.getMilestones(),
    DataAPI.getEmployees(),
    DataAPI.getContacts(),
    DataAPI.getClients(),
    DataAPI.getDeliverables(),
    DataAPI.getTimeEntries()
  ]).then(function (r) {
    return {
      projects: r[0] || [],
      milestones: r[1] || [],
...
function _cxProjectTasks(p, D) {
  return (D.deliverables || []).filter(function (task) {
    return String(_cxRefId(task.projectId)) === String(_cxRefId(p.id));
  });
}

function _cxFindTaskTimer(task, D) {
  var adminId = _cxRefId(STATE.adminUser && STATE.adminUser.id);
  var taskId = _cxRefId(task && task.id);
  var entry = (D.timeEntries || []).find(function (t) {
    return String(_cxRefId(t.deliverableId)) === String(taskId) &&
      String(_cxRefId(t.employeeId)) === String(adminId) &&
      (t.status === 'RUNNING' || t.status === 'PAUSED');
  });
  var base = Number(task && task.loggedSecs || 0);
  ...
  return { seconds: seconds, entry: entry };
}

function _cxChooseProjectTask(p, D) {
  var tasks = _cxProjectTasks(p, D);
  if (!tasks.length) return null;

  var running = tasks.find(function (task) {
    return !!_cxFindTaskTimer(task, D).entry;
  });
  if (running) return running;

  var selected = tasks.find(function (task) {
    return String(_cxRefId(task.id)) === String(_cxRefId(CASES_UI.selectedTaskId));
  });
...
function _cxSelectProjectTask(id) {
  CASES_UI.selectedTaskId = id || null;
  renderCases();
}

function _cxProjectTimerBar(p, D) {
  var esc = Utils.esc;
  var tasks = _cxProjectTasks(p, D);
  var task = _cxChooseProjectTask(p, D);

  if (!task) {
    CASES_UI.selectedTaskId = null;
    return '<div class="cx-project-timer cx-project-timer-empty">' +
      '<div class="cx-project-timer-left"><span class="cx-project-timer-dot"></span><div>' +
      '<strong class="cx-project-timer-title">No task available</strong>' +
      '<span class="cx-project-timer-state">Create or assign a task to start tracking time.</span>' +
      '</div></div><strong id="cx-project-live-timer">00:00:00</strong></div>';
  }

  CASES_UI.selectedTaskId = task.id;
  var timer = _cxFindTaskTimer(task, D);
  var entry = timer.entry;
  var state = entry ? entry.status : (task.status === 'DONE' ? 'DONE' : 'READY');
  var isDone = task.status === 'DONE';
  var buttons = '';

  if (isDone) {
    buttons = '<span class="badge badge-green">✓ Completed</span>';
  } else if (!entry) {
    buttons = '<button class="btn btn-sm btn-green" onclick="_cxTaskStart(' + _cxJsId(task.id) + ', this)">▶ Start Task</button>';
  } else if (entry.status === 'RUNNING') {
    buttons =
      '<button class="btn btn-sm btn-secondary" onclick="_cxTaskPause(' + _cxJsId(entry.id) + ', this)">Ⅱ Pause</button>' +
      '<button class="btn btn-sm btn-danger" onclick="_cxTaskEnd(' + _cxJsId(entry.id) + ', this)">■ End Task</button>';
  } else {
    buttons =
      '<button class="btn btn-sm btn-green" onclick="_cxTaskResume(' + _cxJsId(entry.id) + ', this)">▶ Resume</button>' +
      '<button class="btn btn-sm btn-danger" onclick="_cxTaskEnd(' + _cxJsId(entry.id) + ', this)">■ End Task</button>';
  }

  if (!isDone) {
    buttons += ' <button class="btn btn-sm btn-primary" onclick="_cxTaskComplete(' + _cxJsId(task.id) + ')">✓ Complete</button>';
  }

  var select = '<select class="form-select cx-project-task-select" onchange="_cxSelectProjectTask(this.value)">' +
    tasks.map(function (t) {
      return '<option value="' + esc(String(_cxRefId(t.id))) + '"' +
        (String(_cxRefId(t.id)) === String(_cxRefId(task.id)) ? ' selected' : '') + '>' +
        esc(t.title || 'Untitled task') +
        '</option>';
    }).join('') +
    '</select>';

  var stateLabel = state === 'RUNNING' ? 'Running' : state === 'PAUSED' ? 'Paused' : state === 'DONE' ? 'Completed' : 'Ready';
  var stateClass = state === 'RUNNING' ? 'is-running' : state === 'PAUSED' ? 'is-paused' : '';

  return '<div class="cx-project-timer ' + stateClass + '" data-project-top-task="' + esc(String(_cxRefId(task.id))) + '">' +
    '<div class="cx-project-timer-left">' +
      '<span class="cx-project-timer-dot"></span>' +
      '<div><strong class="cx-project-timer-title">Current Task</strong>' +
      '<span class="cx-project-timer-state">' + esc(stateLabel) + ' · ' + esc(task.title || 'Untitled task') + '</span></div>' +
    '</div>' +
    '<div class="cx-project-timer-controls">' +
      select +
      '<strong id="cx-project-live-timer" data-top-base-secs="' + String(timer.seconds) + '" data-top-entry-status="' + (entry ? entry.status : '') + '" data-top-active-start="' + (entry && entry.activeStartedAt ? esc(entry.activeStartedAt) : '') + '">' + _cxSecondsLabel(timer.seconds) + '</strong>' +
      '<div class="cx-project-timer-buttons">' + buttons + '</div>' +
    '</div>' +
  '</div>';
}
...
function _cxDetail(el, D) {
  var esc = Utils.esc, O = SisOps, people = _cxPeople(D);
  var p = D.projects.filter(function (x) { return String(_cxRefId(x.id)) === String(_cxRefId(CASES_UI.caseId)); })[0];
  if (!p) { cxBackToDash(); return; }
  var client = D.clients.filter(function (c) { return String(_cxRefId(c.id)) === String(_cxRefId(p.clientId)); })[0];
...
function _cxTaskTimer(task, D) {
  ...
  var entry = (D.timeEntries || []).find(function (t) {
    return String(_cxRefId(t.deliverableId)) === String(_cxRefId(task.id)) &&
      String(_cxRefId(t.employeeId)) === String(_cxRefId(STATE.adminUser && STATE.adminUser.id || '')) &&
      (t.status === 'RUNNING' || t.status === 'PAUSED');
  });
  if (!entry) return { seconds: base, entry: null };
...
function _cxSetButtonBusy(button, busy, label) {
  if (!button) return;
...
function _cxTaskStart(id, button) {
  _cxSetButtonBusy(button, true, 'Starting…');

  DataAPI.startTimer(STATE.adminUser && STATE.adminUser.id, id)
    .then(function (entry) {
      if (typeof syncState === 'function') syncState();
      if (entry && entry.action === 'SWITCHED') {
        toast('Previous task paused. New task timer started.', 'success');
      } else if (entry && entry.action === 'RESUMED') {
        toast('Task resumed. Timer is running.', 'success');
      } else {
    ...
    .catch(function (e) {
      _cxSetButtonBusy(button, false);
      toast(e.message || 'Could not start task', 'error');
    });
}

function _cxTaskPause(timerId, button) {
  _cxSetButtonBusy(button, true, 'Pausing…');
  ...
  DataAPI.pauseTimer(timerId)
    ...
    .catch(function (e) {
      _cxSetButtonBusy(button, false);
      toast(e.message || 'Could not pause task', 'error');
    });
}

function _cxTaskResume(timerId, button) {
  _cxSetButtonBusy(button, true, 'Resuming…');
  ...
  DataAPI.resumeTimer(timerId)
    ...
    .catch(function (e) {
      _cxSetButtonBusy(button, false);
      toast(e.message || 'Could not resume task', 'error');
    });
}

function _cxTaskEnd(timerId, button) {
  _cxSetButtonBusy(button, true, 'Ending…');

  DataAPI.stopTimerById(timerId)
    .then(function (result) {
      if (typeof syncState === 'function') syncState();

      var secs = result && (result.activeSeconds || result.seconds)
        ? (result.activeSeconds || result.seconds)
        : 0;

      toast(
        'Task timer stopped. ' +
        _cxSecondsLabel(secs) +
        ' active time saved.',
        'success'
      );
    ...
    .catch(function (e) {
      _cxSetButtonBusy(button, false);
      toast(e.message || 'Could not stop task', 'error');
    });
}

function _cxTaskComplete(id) {
  showConfirm(
    'Mark this task as completed? Any running or paused timer will be stopped and the active time will be saved.',
    function () {
      // Use the backend review endpoint so completion also:
      // 1) stops any open timer,
      // 2) stores active time,
      // 3) marks the task completed,
      // 4) sends the completion email to every owner.
      DataAPI.approveDeliverable(id, 'Completed from the project workspace.')
        .then(function () {
          if (typeof syncState === 'function') syncState();
...
function _cxTaskRows(m, D) {
  ...
  return '<div class="cx-task-list">' + tasks.map(function (task) {
    var owners = (task.assigneeIds || []).map(function (id) {
      var employee = (D.employees || D.users || []).find(function (u) {
        return String(_cxRefId(u.id)) === String(_cxRefId(id));
      });
      ...
      var contact = (D.contacts || []).find(function (c) {
        return String(_cxRefId(c.id)) === String(_cxRefId(id));
      });
    ...
    if (!isDone) {
      if (!entry) {
        actions += '<button class="btn btn-xs btn-green" onclick="_cxTaskStart(' + _cxJsId(task.id) + ', this)">▶ Start Task</button>';
      } else if (timerState === 'RUNNING') {
        actions += '<button class="btn btn-xs btn-secondary" onclick="_cxTaskPause(' + _cxJsId(entry.id) + ', this)">Ⅱ Pause</button>';
        actions += '<button class="btn btn-xs btn-danger" onclick="_cxTaskEnd(' + _cxJsId(entry.id) + ', this)">■ End Task</button>';
      } else {
        actions += '<button class="btn btn-xs btn-green" onclick="_cxTaskResume(' + _cxJsId(entry.id) + ', this)">▶ Resume</button>';
        actions += '<button class="btn btn-xs btn-danger" onclick="_cxTaskEnd(' + _cxJsId(entry.id) + ', this)">■ End Task</button>';
      }
...
function _cxRefreshTaskTimers() {
  ...
  if (projectTimer) {
    var value = Number(projectTimer.getAttribute('data-top-base-secs') || 0);
    if (projectTimer.getAttribute('data-top-entry-status') === 'RUNNING' && projectTimer.getAttribute('data-top-active-start')) {
      value += Math.max(0, Math.round((Date.now() - new Date(projectTimer.getAttribute('data-top-active-start')).getTime()) / 1000));
    }
    projectTimer.textContent = _cxSecondsLabel(value);
  }
}
  ...
  _cxLoad().then(function (D) {
      ...
      if (!W.milestones.length && !W.templateUsed) {
        ...
        body = W.milestones.map(function (m, i) {
          ...
          return '<div class="card card-pad" style="margin-bottom:8px;background:var(--s2)">' +
            ...
            '<div style="margin-top:8px;padding:8px;background:#fff;border:1px solid var(--b1);border-radius:8px">' +
            '<div style="font-size:11px;font-weight:700;margin-bottom:5px">Initial Task (optional)</div>' +
            '<div style="display:flex;gap:6px;flex-wrap:wrap">' +
            '<input class="form-input" style="flex:1;min-width:180px" value="' + esc(m.taskTitle || '') + '" placeholder="Task to create with this milestone" onchange="CASE_WIZ.milestones[' + i + '].taskTitle=this.value">' +
            '<select multiple class="form-select" style="min-width:190px;min-height:72px" onchange="cxWizSetTaskOwners(' + i + ', this)">' +
            ppl.map(function (o) { return '<option value="' + esc(o.k) + '"' + ((m.taskOwnerKeys && m.taskOwnerKeys[o.k]) ? ' selected' : '') + '>' + esc(o.n) + '</option>'; }).join('') +
            '</select></div>' +
            '<div style="font-size:10px;color:var(--t2);margin-top:3px">Select one or more task owners. If none are selected, the milestone owner is used.</div></div>' +
    ...
    /* STEP 6 — REVIEW + CREATE */
      ...
      body = '<table style="font-size:12px">' +
        ...
        '<tr><td style="color:var(--t2);padding:3px 12px 3px 0">Milestones</td><td>' + W.milestones.length + '</td></tr>' +
        '<tr><td style="color:var(--t2);padding:3px 12px 3px 0">Initial tasks</td><td>' + W.milestones.filter(function (m) { return (m.taskTitle || '').trim(); }).length + '</td></tr></table>' +
...
function cxWizSetTaskOwners(i, select) {
  var map = {};
  Array.from(select.selectedOptions || []).forEach(function (option) {
    if (option.value) map[option.value] = true;
  });
  CASE_WIZ.milestones[i].taskOwnerKeys = map;
}

function cxWizAddBlankMs() {
  CASE_WIZ.milestones.push({ title: '', description: '', owner: Object.keys(CASE_WIZ.ownerKeys).filter(function (k) { return CASE_WIZ.ownerKeys[k]; })[0] || 'user:11', dueDate: '', priority: 'HIGH', afterPrev: CASE_WIZ.milestones.length > 0, customFields: [], taskTitle: '', taskOwnerKeys: {} });
  _cxWizRender();
}
function cxWizUseTemplate() {
  ...
  W.milestones = tpl.map(function (t, i) {
    ...
    return { title: t.title, description: t.description, owner: defOwner, dueDate: Utils.isoDate(cursor), priority: t.priority, afterPrev: i > 0, customFields: [], taskTitle: '', taskOwnerKeys: {} };
  });
    ...
    W.milestones.forEach(function (m, i) {
      chain = chain.then(function () {
        ...
        return DataAPI.createMilestone({
          ...
          var taskTitle = (m.taskTitle || '').trim();
          if (!taskTitle) return created;

          var taskKeys = Object.keys(m.taskOwnerKeys || {}).filter(function (k) {
            return m.taskOwnerKeys[k];
          });

          if (!taskKeys.length && m.owner) taskKeys = [m.owner];

          var assigneeIds = taskKeys.map(function (k) {
            var parts = String(k).split(':');
            return parts.length > 1 ? String(parts.slice(1).join(':')) : String(k);
          }).filter(function (id) {
            return /^[a-fA-F0-9]{24}$/.test(id);
          });

          if (!assigneeIds.length) {
            throw new Error('Task "' + taskTitle + '" needs at least one valid owner.');
          }

          return DataAPI.createDeliverable({
            projectId: proj.id,
            ...
            createdById: STATE.adminUser && STATE.adminUser.id ? STATE.adminUser.id : null,
            origin: 'ADMIN',
            dueAt: (m.dueDate || proj.deadline)
              ? (m.dueDate || proj.deadline) + 'T18:00:00'
...
function cxAssignTask(pid, msId) {
  ...
  var m = milestones.find(function (item) {
    return String(item.id) === String(msId);
  }) || {};

  var employeeOptions = employees.map(function (employee) {
    return '<option value="' + Utils.esc(employee.id) + '">' +
      Utils.esc(employee.name || 'Employee') +
      (employee.role ? ' — ' + Utils.esc(employee.role) : '') +
      '</option>';
  }).join('');

  var contactOptions = contacts
    .filter(function (contact) {
      return contact.email;
    })
    .map(function (contact) {
      return '<option value="' + Utils.esc(contact.id) + '">' +
        Utils.esc(contact.name || 'Contact') +
        ' — ' + Utils.esc(contact.email) +
        '</option>';
    }).join('');

  var ownerOptions =
    (employeeOptions
      ? '<optgroup label="Oment Employees">' + employeeOptions + '</optgroup>'
      : '') +
    (contactOptions
  ...
  openModal(
    'Assign Task — ' + Utils.esc(m.title || ''),
    '<div class="cx-modal-section">' +
      '<div class="cx-modal-section-title">Task details</div>' +
      '<div class="cx-form-grid">' +
        '<div class="form-group cx-span-2">' +
          '<label class="form-label">Task title</label>' +
          '<input id="cx-at-title" class="form-input" placeholder="Enter task title" value="' + Utils.esc(m.title || '') + '">' +
        '</div>' +
        '<div class="form-group cx-span-2">' +
          '<label class="form-label">Details</label>' +
          '<textarea id="cx-at-desc" class="form-textarea" placeholder="Describe what needs to be completed">' + Utils.esc(m.description || '') + '</textarea>' +
        '</div>' +
      '</div>' +
    '</div>' +
    '<div class="cx-modal-section">' +
      '<div class="cx-modal-section-title">Owners & deadline</div>' +
      '<div class="cx-form-grid">' +
        '<div class="form-group">' +
          '<label class="form-label">Assign to owners</label>' +
          '<select id="cx-at-who" class="form-select" multiple size="5">' +
            ownerOptions +
          '</select>' +
          '<div class="cx-form-help">Hold Ctrl/Cmd to select multiple owners. Each selected owner receives the assignment email.</div>' +
        '</div>' +
        '<div class="form-group">' +
          '<label class="form-label">Deadline</label>' +
          '<input id="cx-at-due" type="date" class="form-input" value="' + Utils.esc((m.dueDate || '').slice(0, 10)) + '">' +
        '</div>' +
      '</div>' +
    '</div>',
    '<button class="btn" type="button" onclick="closeModal()">Cancel</button>' +
    '<button class="btn btn-primary" type="button" onclick="cxAssignTaskGo(' + _cxJsId(pid) + ',' + _cxJsId(msId) + ', this)">Assign Task</button>'
  );
}

function cxAssignTaskGo(pid, msId, button) {
  _cxSetButtonBusy(button, true, 'Assigning…');

  var ownerSelect = document.getElementById('cx-at-who');
  var ownerIds = ownerSelect
    ? Array.from(ownerSelect.selectedOptions).map(function (option) {
        return option.value;
      }).filter(Boolean)
    : [];

  var title = ((document.getElementById('cx-at-title') || {}).value || '').trim();
  ...
  if (!ownerIds.length) {
    _cxSetButtonBusy(button, false);
    toast('Please select at least one owner.', 'error');
    return;
  }
  ...
  DataAPI.createDeliverable({
    ...
    createdById: STATE.adminUser && STATE.adminUser.id
      ? STATE.adminUser.id
      : null,
    origin: 'ADMIN',
    dueAt: due
    ...
    .then(function (task) {
      CASES_UI.selectedTaskId = task && task.id ? task.id : null;
      ...
      closeModal();
      toast(
        'Task assigned to ' + ownerIds.length + ' owner' +
        (ownerIds.length === 1 ? '' : 's') + '.',
        'success'
      );

      // Immediately show the newly-created task under its milestone.
      renderCases();
    })
    .catch(function (error) {
      _cxSetButtonBusy(button, false);
      toast(error.message || 'Could not assign task.', 'error');
    });
}
