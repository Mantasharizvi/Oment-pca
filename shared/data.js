...
(function(root){
  ...
  var apiRequestsPending = 0;

  function apiProgressStart(){
    apiRequestsPending += 1;
    if (!root.document) return;
    var bar = root.document.getElementById('api-progress');
    if (!bar) return;
  ...
  function apiProgressEnd(){
    apiRequestsPending = Math.max(0, apiRequestsPending - 1);
    if (apiRequestsPending !== 0 || !root.document) return;
    var bar = root.document.getElementById('api-progress');
    if (!bar) return;
    bar.classList.remove('on');
    bar.classList.add('done');
    root.setTimeout(function(){
      if (apiRequestsPending === 0) bar.classList.remove('done');
    }, 220);
  }
  ...
  async function request(method,path,body,opts){
    ...
    try {
      ...
      if(r.status===401){
        ...
        CURRENT=null;
        try{
          root.localStorage.removeItem(CURRENT_KEY);
        }catch(e){}
      }
  ...
  var DataAPI={
    ...
    startTimer:function(employeeId,deliverableId){return request('POST','/timers/start',{deliverableId:deliverableId||null}).then(function(d){
      // The backend may automatically pause the owner's previously running
      // task before starting this one. Keep both timer records in sync.
      if(d&&d.switchedFrom){
        updateCache('timeEntries',d.switchedFrom);
        if(d.switchedFrom.deliverableId){
          var previousTask=cache.deliverables.find(function(x){return String(x.id)===String(d.switchedFrom.deliverableId);});
          if(previousTask&&previousTask.status!=='DONE')previousTask.status='IN_PROGRESS';
        }
      }
      updateCache('timeEntries',d);
      if(deliverableId){var task=cache.deliverables.find(function(x){return String(x.id)===String(deliverableId);});if(task)task.status='IN_PROGRESS';}
      return clone(d);
    });},
    ...
    stopTimerById:function(timerId){
      return request('POST','/timers/'+encodeURIComponent(timerId)+'/stop',{})
        .then(function(d){
          if(d && d.timeEntry){
            updateCache('timeEntries', d.timeEntry);

            if(d.timeEntry.deliverableId){
              var task = cache.deliverables.find(function(x){
                return String(x.id) === String(d.timeEntry.deliverableId);
              });

              if(task){
                task.loggedSecs = Number(task.loggedSecs || 0) + Number(d.activeSeconds || d.seconds || 0);
              }
            }

            if(d.timeEntry.subtaskId){
              var subtask = cache.subtasks.find(function(x){
                return String(x.id) === String(d.timeEntry.subtaskId);
              });

              if(subtask){
                subtask.loggedSecs = Number(subtask.loggedSecs || 0) + Number(d.activeSeconds || d.seconds || 0);
              }
            }
          }
    ...
    getTimeEntries:function(filter){return listResource('timeEntries',filter||{});},
