...
function startT(task){
  ...
  if(CU){
    DataAPI.startTimer(CU.id,task.id).then(function(entry){
      // Starting another task automatically pauses the previously running
      // task on the backend. Reflect that switch in the employee UI too.
      if(entry&&entry.switchedFrom){
        var previous=TASKS.find(function(t){return String(t.id)===String(entry.switchedFrom.deliverableId);});
        if(previous){
          previous.timerStatus='PAUSED';
          previous.timerId=entry.switchedFrom.id;
          previous.active=false;
          previous.logged=Number(entry.switchedFrom.totalActiveSecs||previous.logged||0);
        }
      }
      task.timerId=entry&&entry.id?entry.id:task.timerId;
      task.timerStatus='RUNNING';
      task.active=true;
      if(entry&&entry.action==='SWITCHED')toast('Previous task paused. This task is now running.','ok');
    }).catch(function(err){
